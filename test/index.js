...
ava.test("front pressed correctly is stored correctly", function(test) {
  ...
  test.true(
    spy.calledWith(
      sinon.match.has(
        "Key",
        sinon.match.has("frontId", sinon.match.has("S", "myFront"))
      )
    )
  );
  ...
  test.true(
    spy.calledWith(
      sinon.match.has(
        ...
        sinon.match.has(":time", sinon.match.has("S", date))
      )
    )
  );
});
...
ava.test(
  "send email when error count is above threshold on PROD and live",
  function(test) {
    test.plan(3);
...
ava.test(
  "does not send email on CODE even when error count is above threshold",
  function(test) {
    test.plan(2);
...
ava.test(
  "does not send email on DRAFT even when error count is above threshold",
  function(test) {
    test.plan(2);
...
ava.test("send email when seeing a new error", function(test) {
  ...
  test.true(
    dynamoPutSpy.calledWith(
      sinon.match.has(
        "Item",
        sinon.match.has("error", sinon.match.has("S", "error"))
      )
    )
  );
  test.true(
    dynamoPutSpy.calledWith(
      sinon.match.has(
        ...
        sinon.match.has(
          "lastSeen",
          sinon.match.has("N", today.valueOf().toString())
        )
      )
    )
  );
    ...
    getItem: function(record, callback) {
      callback(null, {
        Item: {
          error: { S: "error" },
          lastSeen: {
            N: new Date().setMinutes(today.getMinutes() - 60).toString()
          },
          affectedFronts: { SS: ["testFront"] }
        }
      });
